...
const ProfileScreen = ({ route, navigation }) => {
  ...
  return (
    <ProfileContainer>
      ...
      <ScrollView scrollEventThrottle={16}>{gridView(images, width, height)}</ScrollView>
    </ProfileContainer>
  );
};
...
export const gridView = (images, width, height) => {
  return <View style={{ flexDirection: 'row', flexWrap: 'wrap' }}>{renderImages(images, width, height)}</View>;
};
  ...
  return images.map((image) => {
    return (
      <View style={[{ width: width / 3 }, { height: width / 3 }]}>
        <Image
          style={{ flex: 1, width: undefined, height: undefined, borderColor: '#FFFFFF', borderWidth: 1 }}
          source={image}
        />
      </View>
    );
